...
export default function BalanceSummary({ balance, assignments, completedToday, onCompleteDay }: Props) {
  ...
  const afterComplete = Math.max(0, totalRemaining - totalPlanned);

  const canComplete = !completedToday && totalPlanned > 0;
        ...
        {PRAYERS.map(p => {
          ...
          return (
            <View key={p} style={styles.prayerCol}>
              ...
              <Text style={[styles.prayerCount, { color: rem === 0 ? 'rgba(255,255,255,0.2)' : c.hex }]}>
                {rem}
              </Text>
      ...
      <View style={styles.row}>
        <View>
          ...
          <View style={styles.totalRow}>
            ...
            {totalPlanned > 0 && (
              <Text style={styles.totalAfter}>→ {afterComplete.toLocaleString()}</Text>
            )}
          </View>
        </View>
        ...
        <TouchableOpacity
          ...
          style={[
            ...
            !canComplete && !completedToday && styles.btnDisabled,
          ]}
        >
          <Text style={[styles.btnText, completedToday && { color: '#a78bfa' }]}>
